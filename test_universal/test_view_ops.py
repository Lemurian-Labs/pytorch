...
from torch.testing._internal.common_dtype import (
    get_all_dtypes, get_all_fp_dtypes, get_all_complex_dtypes,
    native_equivalent, universal_types, get_all_dtypes_and_universal
)
...
class TestViewOps(TestCase):
    ...
    def test_view_dtype_new(self, device, dtype):
        ...
        for a in generate_inputs():
            if dtype in universal_types():
                a_np = a.to(native_equivalent[dtype]).cpu().numpy()
                a_np_contiguous = a.to(native_equivalent[dtype]).cpu().contiguous().numpy()
            else:
                a_np = a.cpu().numpy()
        ...
        if dtype.is_floating_point or dtype.is_complex:
            for view_dtype in [*get_all_fp_dtypes(), *get_all_complex_dtypes(), *universal_types()]:
    ...
    def test_view_dtype_upsize_errors(self, device, dtype):
        ...
        for view_dtype in get_all_dtypes_and_universal():
            view_dtype_size = torch._utils._element_size(view_dtype)
            ...
            for n in range(ndims + 1):
                for with_extremal in [False, True]:
                    for contiguous in [False, True]:
                        ...
                        if dtype in universal_types():
                            np_sequence_x = tuple(
                                np.array(x.detach().to(native_equivalent[dtype]).cpu().numpy())
                                for x in torch_sequence_x
                            )
                        else:
                        ...
                        if dtype in universal_types():
                            torch_res = tuple(x.to(native_equivalent[dtype]).cpu() for x in torch_res)
                        else:
    ...
    def test_broadcast_to(self, device, dtype):
        ...
        for s0, s1 in combinations(sizes, r=2):
            ...
            if can_broadcast(s0, s1):
                ...
                if dtype in universal_types():
                    res = res.to(native_equivalent[dtype])
    ...
    def test_tensor_split_sections(self, device, dtype):
        ...
        for input_size in input_sizes:
            ...
            for a in [a_base, a_base.t()] if a_base.dim() > 2 else [a_base]:
                if dtype in universal_types():
                    a_n = a.to(native_equivalent[dtype]).cpu().numpy()
                else:
                    a_n = a.cpu().numpy()
                for dim in range(-a.dim(), a.dim()):
                    for sections in range(1, 2 * a.size(dim)):
                        ...
                        if dtype in universal_types():
                            result1 = tuple(r.to(native_equivalent[dtype]) for r in result1)
                            result2 = tuple(r.to(native_equivalent[dtype]) for r in result2)
            ...
            for a in [a_base, a_base.t()] if a_base.dim() > 2 else [a_base]:
                ...
                for dim in range(-a.dim(), a.dim()):
                    for indices in indices_args:
                        ...
                        if dtype in universal_types():
                            result_1 = tuple(r.to(native_equivalent[dtype]) for r in result_1)
                            result_2 = tuple(r.to(native_equivalent[dtype]) for r in result_2)
