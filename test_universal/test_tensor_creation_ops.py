...
from torch.testing._internal.common_utils import (
    TestCase, run_tests, do_test_empty_full, suppress_warnings,
...
from torch.testing._internal.common_dtype import (
    get_all_math_dtypes, get_all_dtypes, native_equivalent, universal_fp_limit,
    universal_types, get_all_dtypes_and_universal
)
...
class TestTensorCreation(TestCase):
    ...
    def test_vander_types(self, device, dtype):
        ...
        for x, n, inc in product(X, N, increasing):
            numpy_dtype = torch_to_numpy_dtype_dict[native_equivalent[dtype]]
            ...
            pt_res = pt_res.to(native_equivalent[dtype])
    ...
    def test_cat_all_dtypes_and_devices(self, device):
        for dt in get_all_dtypes_and_universal():
    ...
    def test_triu_tril(self, device):
        ...
        def run_test(shape, device, diagonal, dtype):
            ...
            for upper in [True, False]:
                ...
                if dtype in universal_types():
                    x_numpy = x.to(native_equivalent[dtype]).to('cpu').numpy()
                    res1_numpy = res1.to(native_equivalent[dtype]).cpu().numpy()
                else:
                    x_numpy = x.to('cpu').numpy()
                    res1_numpy = res1.cpu().numpy()
                self.assertEqual(numpy_tri_func(x_numpy, diagonal), res1_numpy)
        ...
        dtypes = [dtype for dtype in get_all_dtypes_and_universal() if dtype != torch.bfloat16]
    ...
    def _hvd_split_helper(self, torch_fn, np_fn, op_name, inputs, device, dtype, dim):
        ...
        for shape, arg in inputs:
            ...
            if dtype in universal_types():
                t_np = t.to(native_equivalent[dtype]).cpu().numpy()
            else:
                t.cpu().numpy()

            if not error_expected:
                torch_result = torch_fn(t, arg)
                if dtype in universal_types():
                    torch_result = tuple(result.to(native_equivalent[dtype]) for result in torch_result)
                np_result = np_fn(t_np, arg)
    ...
    def _test_special_stacks(self, dim, at_least_dim, torch_fn, np_fn, device, dtype):
        ...
        if dtype in universal_types():
            actual = actual.to(native_equivalent[dtype])
        expected = np_fn([
            input.to(native_equivalent[dtype]).cpu().numpy() if dtype in universal_types() else input.cpu().numpy()
            for input in input_t
        ])
    ...
    def test_hstack_column_stack(self, device, dtype):
        ...
        torch_result = torch.column_stack(inputs).to(native_equivalent[dtype])

        np_inputs = [input.to(native_equivalent[dtype]).cpu().numpy() for input in inputs]
    ...
    def test_vstack_row_stack(self, device, dtype):
        ...
        for torch_op, np_op in ops:
            ...
            for i in range(5):
                ...
                np_input = [input.to(native_equivalent[dtype]).cpu().numpy() for input in torch_input]
                actual = torch_op(torch_input).to(native_equivalent[dtype])
    ...
    def test_dstack(self, device, dtype):
        ...
        for i in range(5):
            ...
            np_input = [input.to(native_equivalent[dtype]).cpu().numpy() for input in torch_input]
            actual = torch.dstack(torch_input).to(native_equivalent[dtype])
    ...
    def test_random_full_range(self, device, dtype):
        ...
        if dtype in universal_types():
            fp_limit = universal_fp_limit[dtype]
    ...
    def test_random_default(self, device, dtype):
        ...
        if dtype in universal_types():
            to_inc = universal_fp_limit[dtype]
        elif dtype == torch.float:
            to_inc = 1 << 24
        elif dtype == torch.double:
            to_inc = 1 << 53
    ...
    def test_eye(self, device):
        for dtype in get_all_dtypes_and_universal():
            if dtype == torch.bfloat16:
                continue
    ...
    def test_linspace_vs_numpy(self, device, dtype):
        ...
        for steps in [1, 2, 3, 5, 11, 256, 257, 2**22]:
            ...
            a = np.linspace(start, end, steps, dtype=torch_to_numpy_dtype_dict[native_equivalent[dtype]])
            t = t.to(native_equivalent[dtype]).cpu()
            self.assertEqual(t, torch.from_numpy(a))
    ...
    def test_logspace_vs_numpy(self, device, dtype):
        ...
        for steps in [1, 2, 3, 5, 11, 256, 257, 2**22]:
            ...
            a = np.logspace(start, end, steps, dtype=torch_to_numpy_dtype_dict[native_equivalent[dtype]])
            t = t.to(native_equivalent[dtype]).cpu()
            self.assertEqual(t, torch.from_numpy(a))
    ...
    def test_tensor_factories_empty(self, device):
        ...
        for shape in shapes:
            for dt in get_all_dtypes_and_universal():
                ...
                if dt not in {torch.double, torch.float, torch.half, torch.bfloat16, torch.complex64, torch.complex128} and dt not in universal_types():
                    self.assertRaises(RuntimeError, lambda: torch.rand(shape, device=device, dtype=dt).shape)

                if dt == torch.double or dt == torch.float or dt.is_complex or dt in universal_types():
...
def identity(tensor):
    return tensor
def to_numpy(tensor):
    if tensor.dtype in universal_types():
        return tensor.to(native_equivalent[tensor.dtype]).numpy()
    else:
        return tensor.numpy()
...
class TestAsArray(TestCase):
    ...
    def _test_copy_with_cvt(self, cvt, device, dtype, shape=(5, 5), only_with_dtype=False):
        ...
        if not only_with_dtype:
            for other in get_all_dtypes_and_universal():
                if dtype != other:
