...
from torch.testing._internal.common_dtype import native_equivalent, universal_types
...
class TestShapeOps(TestCase):
    ...
    def test_diagonal_multidim(self, device, dtype):
        ...
        if dtype in universal_types():
            xn = x.to(native_equivalent[dtype]).numpy()
        else:
            xn = x.numpy()
        for args in [(2, 2, 3),
                     (2,),
            ...
            expected = xn.diagonal(*args).copy()
            self.assertEqual(expected.shape, result.shape)
            if dtype in universal_types():
                result = result.to(native_equivalent[dtype])
            self.assertEqual(expected, result)
        # test non-continguous
        xp = x.permute(1, 2, 3, 0)
        result = torch.diagonal(xp, 0, -2, -1)
        if dtype in universal_types():
            expected = xp.to(native_equivalent[dtype]).numpy().diagonal(0, -2, -1)
        else:
    ...
    def test_trace(self, device, dtype):
        def test(shape):
            ...
            if dtype in universal_types():
                np_array = tensor.to(native_equivalent[dtype]).cpu().numpy()
            else:
                np_array = tensor.cpu().numpy()

            result = np.trace(np_array, dtype=expected_dtype)
            expected = torch.tensor(result, device=device)
            tensor = tensor.trace()
            if dtype in universal_types():
                tensor = tensor.to(native_equivalent[dtype])
    ...
    def generate_clamp_baseline(self, device, dtype, *, min_vals, max_vals, with_nans):
        ...
        if dtype in universal_types():
            X_numpy = X.to(native_equivalent[dtype]).cpu().numpy()
        else:
            X_numpy = X.cpu().numpy()
        X_clamped = torch.tensor(np.clip(X_numpy, a_min=min_vals, a_max=max_vals), device=device)
        return X, X_clamped
    ...
    def test_clamp(self, device, dtype):
        ...
        for op in op_list:
            for min_val, max_val in args:
                ...
                if dtype in universal_types():
                    Y_actual = Y_actual.to(native_equivalent[dtype])
                ...
                if op in (torch.clamp, torch.clip):
                    ...
                    if dtype in universal_types():
                        Y_out = Y_out.to(native_equivalent[dtype])
